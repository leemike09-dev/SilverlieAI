...
export default function SOSScreen({ navigation, route }: Props) {
  ...
  const sendSMSToAll = useCallback((familyList: FamilyMember[], senderName: string) => {
    ...
    Linking.openURL(`sms:${phones.join(separator)}?body=${msg}`).catch(() => {});
    setSmsSent(true);
  }, []);
  ...
  const startCountdown = () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (c > 0) {
        ...
        setTimeout(() => {
          // SMS 발송 후 119 연결
          if (family.length > 0) sendSMSToAll(family, name || '');
          setTimeout(() => Linking.openURL('tel:119'), family.length > 0 ? 1200 : 800);
        }, 800);
      }
    }, 1000);
  };
    ...
    <View style={s.root}>
      ...
      <View style={s.body}>
        ...
        <View style={s.cdWrap}>
          ...
          {smsSent && (
            <Text style={s.smsSentTxt}>✉️ 가족에게 문자를 보냈어요</Text>
          )}
        </View>
