...
export default function FamilyConnectScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (DEMO_MODE) {
        // 데모: 연결된 멤버가 있는 것으로 간주 → Dashboard 바로 이동
        navigation.replace('FamilyDashboard', { userId: 'demo-user', name: uname });
        return;
      }
  ...
  const shareKakao = () => {
    if (!myCode) return;
    Alert.alert('카카오톡 공유', `Silver Life AI 가족 연결 코드: ${myCode}`, [{ text: '확인' }]);
  };
  ...
  const joinFamily = async () => {
    ...
    try {
      const r = await fetch(`${API}/family/join`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ family_id: userId, family_name: name, link_code: joinCode.replace(/-/g, '') }),
      });
      const d = await r.json();
      if (r.ok) {
  ...
  const saveRelation = async (rel: typeof RELATIONS[0]) => {
    ...
    // AsyncStorage에 관계 저장
    await AsyncStorage.setItem(`relation_${pendingConn.seniorId}`, rel.key);
    try {
      await fetch(`${API}/family/relation`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ family_id: userId, senior_id: pendingConn.seniorId, connection_id: pendingConn.connectionId, relation: rel.key, relation_label: rel.label }),
      });
    } catch {}
    navigation.replace('FamilyDashboard', { userId, name, seniorId: pendingConn.seniorId, seniorName: pendingConn.seniorName, seniorRelation: rel.key });
  };

  const skipRelation = () => {
    setRelModal(false);
    if (!pendingConn) return;
    navigation.replace('FamilyDashboard', { userId, name, seniorId: pendingConn.seniorId, seniorName: pendingConn.seniorName });
  };
    ...
    <View style={s.root}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        <View style={s.section}>
          ...
          {myCode ? (
            <>
              <View style={s.codeBox}><Text style={s.codeText}>{myCode}</Text></View>
        ...
        <View style={s.divider}>
          <View style={s.divLine} /><Text style={s.divTxt}>또는</Text><View style={s.divLine} />
        </View>
        ...
        <View style={s.section}>
          ...
          <TextInput
            style={s.codeInput} placeholder="6자리 코드 입력" placeholderTextColor="#BDBDBD"
            value={joinCode} onChangeText={t => { setJoinCode(t); setMsg(''); }}
            keyboardType="number-pad" maxLength={6}
          />
          {msg ? <Text style={[s.msgTxt, msg.includes('완료') ? s.msgOk : s.msgErr]}>{msg}</Text> : null}
      ...
      <Modal visible={relModal} transparent animationType="slide" onRequestClose={skipRelation}>
        <View style={s.overlay}>
          <View style={s.modalSheet}>
            ...
            {pendingConn && <Text style={s.modalSub}>{pendingConn.seniorName}님과의 관계를 선택해주세요</Text>}
            <View style={s.relGrid}>
              {RELATIONS.map(rel => (
                <TouchableOpacity key={rel.key} style={s.relBtn} onPress={() => saveRelation(rel)} activeOpacity={0.8}>
...
const s = StyleSheet.create({
  ...
  relBtn:      { width: '30%', backgroundColor: '#F0F5FF', borderRadius: 16, paddingVertical: 18, alignItems: 'center', borderWidth: 1.5, borderColor: '#D0DCF0' },
