...
export default function AIChatScreen({ route, navigation }: Props) {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      setMsgs(prev => [...prev, { role: 'ai', text: reply, riskLevel, doctorMemoNeeded, doctorMemo }]);
    ...
    <View style={s.root}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView ref={scrollRef} style={s.chatArea} contentContainerStyle={s.chatContent}
          ...
          {msgs.map((m, i) => (
            m.role === 'ai' ? (
              <View key={i} style={s.aiRow}>
                <Image source={getKkulbiImage(m.riskLevel, m.text)} style={s.aiAvatar} resizeMode="contain" />
                <View style={{ flex: 1 }}>
                  ...
                  {m.doctorMemoNeeded && m.doctorMemo && (
                    <TouchableOpacity style={s.doctorMemoBtn}
                      onPress={() => saveDoctorMemo(m.doctorMemo!)} activeOpacity={0.8}>
          ...
          {loading && (
            <View style={s.aiRow}>
              <Image source={KKULBI_IMAGES.cheer} style={s.aiAvatar} resizeMode="contain" />
              <View style={s.aiBubble}>
...
const s = StyleSheet.create({
  ...
  beeWrap: { width: 120, height: 120, borderRadius: 60, overflow: 'hidden',
    borderWidth: 3, borderColor: '#fff', shadowColor: C.purple1, shadowOpacity: 0.2,
  ...
  aiBubble: { backgroundColor: C.card, borderRadius: 4, borderTopLeftRadius: 18,
    borderTopRightRadius: 18, borderBottomRightRadius: 18, padding: 14,
  ...
  bannerHigh: { backgroundColor: '#FFF0E0', borderRadius: 10, borderWidth: 2, borderColor: '#E65100',
    paddingHorizontal: 12, paddingVertical: 10, marginBottom: 6 },
  bannerHighTxt: { fontSize: 16, color: '#E65100', fontWeight: '700', textAlign: 'center' },
  bannerMedium: { backgroundColor: C.warnBg, borderRadius: 10, borderWidth: 1.5, borderColor: C.warn,
  ...
  doctorMemoBtn: { marginTop: 10, backgroundColor: '#E8F5E9', borderRadius: 12,
    borderWidth: 1.5, borderColor: '#43A047', paddingVertical: 12, paddingHorizontal: 16,
    alignItems: 'center' },
  doctorMemoBtnTxt: { fontSize: 18, color: '#2E7D32', fontWeight: '800' },
  userRow:    { flexDirection: 'row', justifyContent: 'flex-end', marginBottom: 14 },
  userBubble: { backgroundColor: C.purple2, borderRadius: 18, borderBottomRightRadius: 4,
  ...
  micBtn:       { width: 44, height: 44, borderRadius: 22, backgroundColor: C.purpleCard,
  ...
  micIcon:      { fontSize: 20 },
  sendBtn:    { width: 44, height: 44, borderRadius: 22, backgroundColor: C.purple2,
  ...
  btn119: { backgroundColor: C.emRed, borderRadius: 14, paddingVertical: 16, paddingHorizontal: 24,
    width: '100%', alignItems: 'center', marginBottom: 12 },
  btn119Txt: { fontSize: 22, fontWeight: '800', color: '#fff' },
  btnFamily: { backgroundColor: C.purple1, borderRadius: 14, paddingVertical: 14, paddingHorizontal: 24,
    width: '100%', alignItems: 'center', marginBottom: 12 },
