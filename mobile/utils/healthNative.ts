...
async function readAndroidData(): Promise<HealthNativeData> {
  ...
  try {
    ...
    const hrResult = await HC.readRecords('HeartRate', range(startISO)).catch(() => ({ records: [] }));
    ...
    const spo2 = spo2Records.length > 0
      ? Math.round(spo2Records[spo2Records.length - 1].percentage * 100)
      : null;
    ...
    return {
      connected: hrValues.length > 0 || spo2 !== null || sleepMs > 0,
